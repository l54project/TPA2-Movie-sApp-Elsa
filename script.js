const movies = [
  ...
  function search() {
    ...
    searchVal = elInput.value || ''
    console.log(searchVal)
    const dataMovies = searchVal ? movies.filter(m => m.title.trim().toUpperCase().includes(searchVal.trim().toUpperCase())) : movies
    const elListMovies = document.getElementById('list-movies')
    const elNotFound = document.getElementById('msg-not-found')
    if (!dataMovies.length) {
      elNotFound.classList.remove('d-none')
    } else {
      elNotFound.classList.add('d-none')
    }
    ...
    elListMovies.innerHTML = moviesHTML
  }
